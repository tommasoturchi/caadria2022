        ...
        <Box sx={{ m: 1, position: "fixed", bottom: "6rem", left: "2rem" }}>
          <InfoBox>
            <Typography variant="subtitle2" component="div" color="primary">
              {`Buildings: ${
                clusters == null || clusters[0].length === 0 ? "—" : clusters[0].map(c => `${c.toFixed(2)}km`).join(", ")
              }`}
            </Typography>
            <Typography variant="subtitle2" component="div" color="primary">
              {`Green Areas: ${
                clusters == null || clusters[1].length === 0 ? "—" : clusters[1].map(c => `${c.toFixed(2)}km`).join(", ")
              }`}
            </Typography>
            <Typography variant="subtitle2" component="div" color="primary">
              {`Infrastructures: ${
                clusters == null || clusters[2].length === 0 ? "—" : clusters[2].map(c => `${c.toFixed(2)}km`).join(", ")
              }`}
            </Typography>
            <Typography variant="subtitle2" component="div" color="primary">
              {`Natural Elements: ${
                clusters == null || clusters[3].length === 0 ? "—" : clusters[3].map(c => `${c.toFixed(2)}km`).join(", ")
              }`}
            </Typography>
          </InfoBox>
        </Box>
